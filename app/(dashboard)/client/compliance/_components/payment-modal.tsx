...
import { useCreatePaymentIntentMutation } from "@/lib/api/stripePaymentApi";
...
interface PaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  compliancePriceNGN: number;  // Nigerian price in NGN
  compliancePriceGBP: number;  // UK price in GBP
...
export function PaymentModal({
  ...
  const handlePaymentInitiation = async () => {
    ...
    try {
      const reference = `COMPLIANCE-${userId}-${Date.now()}`;
      const currentUrl = typeof window !== "undefined" ? window.location.origin : "";

      if (jurisdiction === "NG") {
        // Nigeria - Paystack
        const response = await initializePayment({
          amount: Math.floor(compliancePriceNGN * 100), // Paystack uses smallest currency unit
          email: userEmail,
          currency: "NGN",
          paymentType: "compliance_passport",
          callbackUrl: `${currentUrl}/compliance/payment/callback`,
          metadata: JSON.stringify({
            userId,
            type: "compliance_passport",
            jurisdiction: "NG",
          }),
        }).unwrap();
        ...
        const response = await createPaymentIntent({
          amount: Math.floor(compliancePriceGBP * 100),
          currency: "GBP",
          reference,
          paymentType: "compliance_passport",
          email: userEmail,
          description: "Compliance Passport Generation",
          returnUrl: `${currentUrl}/compliance/payment/stripe-return`,
          metadata: {
        ...
        // Success for Stripe - would normally open payment UI in real implementation
        onPaymentSuccess(reference, jurisdiction);
        setStatus("success");
        toast({
  ...
  return (
    <>
      {/* Backdrop — HIGHER z-index than PassportSheet's backdrop */}
      <div
        onClick={handleClose}
        style={{
          position: "fixed",
      ...
      {/* Panel — HIGHER z-index than PassportSheet's panel */}
      <div
        style={{
          position: "fixed",
          top: 0,
        ...
        >
          <div>
            <h2 style={{ color: "white", fontWeight: 700, fontSize: "17px", margin: 0 }}>
              {status === "success" ? "Payment Initiated" : "Generate Compliance Passport"}
            </h2>
            <p style={{ color: "#6B7280", fontSize: "13px", marginTop: "4px" }}>
              {status === "success" ? "You'll be redirected shortly" : "Select your jurisdiction and complete payment"}
            </p>
          </div>
          <button
            onClick={handleClose}
        ...
        >
          {status === "success" ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "200px", gap: "16px" }}>
              ...
              <p style={{ textAlign: "center", color: "rgba(255,255,255,0.7)" }}>Payment successful! Generating your passport...</p>
            </div>
          ) : (
            <>
              ...
              <div style={{ display: "flex", gap: "12px" }}>
                {[
                  { code: "GB", name: "United Kingdom", price: compliancePriceGBP, currency: "£", gateway: "Stripe" },
                  { code: "NG", name: "Nigeria", price: compliancePriceNGN, currency: "₦", gateway: "Paystack" },
                ].map((option) => (
                  <button
                    key={option.code}
                  ...
                  >
                    ...
                    <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)" }}>{option.gateway}</div>
                  </button>
                ))}
              </div>
              ...
              >
                ...
                <span>Payment is required to generate your compliance passport. Your passport is valid for 4 years.</span>
              </div>
            </>
          )}
        </div>
