...
export default function CompliancePaymentCallbackPage() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCountdown((c) => {
        if (c <= 1) {
          clearInterval(timer);
          router.replace("/accubridge/client/compliance");
          return 0;
        }
        return c - 1;
      });
        ...
        {isLoading || !reference ? (
          <>
            ...
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: BRAND.muted }}>Please wait while we confirm your payment…</p>
            </div>
          </>
        ) : isError || !success ? (
          <>
