...
import { useConnectTaxMutation } from "@/lib/api/complianceCentreApi";
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        setTimeout(() => router.replace("/client/compliance?hmrc=connected"), 2500);
      } catch {
        setStatus("error");
        setMessage("HMRC authorisation received, but we could not complete the connection. Please try again.");
        setTimeout(() => router.replace("/client/compliance?hmrc=error"), 3000);
      }
    };
      ...
      {status === "error" && (
        <>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => router.push("/client/compliance")}
