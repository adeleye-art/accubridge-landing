...
const STAFF_NAV: NavSection[] = [
  {
    title: "Main",
    items: [
      { id: "dashboard",    label: "Dashboard",    href: "/accubridge/staff/dashboard",    icon: <LayoutDashboard size={18} /> },
      { id: "clients",      label: "My Clients",   href: "/accubridge/staff/clients",      icon: <Users size={18} /> },
      { id: "transactions", label: "Transactions", href: "/accubridge/staff/transactions", icon: <ArrowLeftRight size={18} /> },
      { id: "reports",      label: "Reports",      href: "/accubridge/staff/reports",      icon: <BarChart3 size={18} /> },
      { id: "compliance",   label: "Compliance",   href: "/accubridge/staff/compliance",   icon: <ShieldCheck size={18} /> },
      { id: "notes",        label: "Notes",        href: "/accubridge/staff/notes",        icon: <StickyNote size={18} /> },
      { id: "documents",    label: "Documents",    href: "/accubridge/staff/documents",    icon: <FileText size={18} /> },
      { id: "funding",      label: "Funding Review", href: "/accubridge/staff/funding",    icon: <Landmark size={18} /> },
    ],
  },
  {
    title: "Account",
    items: [
      { id: "settings", label: "Settings", href: "/accubridge/staff/settings", icon: <Settings size={18} /> },
    ],
  },
];

// ─── Path → item mapping ──────────────────────────────────────────────────────
const STAFF_PATH_TO_ITEM: [string, string][] = [
  ["/accubridge/staff/clients",      "clients"],
  ["/accubridge/staff/transactions", "transactions"],
  ["/accubridge/staff/reports",      "reports"],
  ["/accubridge/staff/compliance",   "compliance"],
  ["/accubridge/staff/notes",        "notes"],
  ["/accubridge/staff/documents",    "documents"],
  ["/accubridge/staff/funding",      "funding"],
  ["/accubridge/staff/settings",     "settings"],
  ["/accubridge/staff/dashboard",    "dashboard"],
];

function staffItemFromPath(pathname: string): string {
  for (const [prefix, id] of STAFF_PATH_TO_ITEM) {
  ...
  return (
    <div className="relative w-full flex justify-center">
      <button
        ...
        style={{ backgroundColor: isActive ? `${BRAND.gold}20` : hovered ? "rgba(255,255,255,0.12)" : "transparent", transitionTimingFunction: softSpring }}
...
function SubItemRow({ subItem, isActive }: { subItem: SubItem; isActive: boolean }) {
  ...
  return (
    ...
    >
      <span style={{ color: isActive ? BRAND.gold : hovered ? "#fff" : BRAND.muted, transition: "color 0.15s", display: "inline-flex", flexShrink: 0 }}>{subItem.icon}</span>
      <span className="text-xs font-medium truncate" style={{ color: isActive ? BRAND.gold : hovered ? "#fff" : "rgba(255,255,255,0.6)", transition: "color 0.15s" }}>{subItem.label}</span>
    </button>
  );
}
...
function NavItemRow({ item, isActive, isSubOpen, onToggle, pathname }: { item: NavItem; isActive: boolean; isSubOpen: boolean; onToggle: () => void; pathname: string }) {
  ...
  return (
    <div className="w-full">
      ...
      >
        {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 rounded-r-full" style={{ backgroundColor: BRAND.gold }} />}
        <span style={{ color: isActive ? BRAND.gold : hovered ? "#fff" : "rgba(255,255,255,0.5)", transition: "color 0.15s", display: "inline-flex", flexShrink: 0 }}>{item.icon}</span>
        <span className="flex-1 text-sm font-medium text-left truncate" style={{ color: isActive ? BRAND.gold : hovered ? "#fff" : "rgba(255,255,255,0.85)", transition: "color 0.15s" }}>{item.label}</span>
        {item.badge && (
          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0" style={{ backgroundColor: item.badgeColor || BRAND.accent, color: BRAND.primary }}>{item.badge}</span>
        )}
        {hasSubItems && (
      ...
      {hasSubItems && (
        <div className="overflow-hidden" style={{ maxHeight: isSubOpen ? `${item.subItems!.length * 40 + 8}px` : "0px", opacity: isSubOpen ? 1 : 0, transition: `max-height 0.3s ${softSpring}, opacity 0.25s ease` }}>
          <div className="pt-1 pb-2 flex flex-col gap-0.5">
            {item.subItems!.map((sub) => <SubItemRow key={sub.href} subItem={sub} isActive={pathname === sub.href} />)}
          </div>
        </div>
      )}
    </div>
          ...
          <div key={section.title} className="flex flex-col gap-1">
            ...
            {items.map((item) => (
              <NavItemRow
                ...
                onToggle={() => { onSetActive(item.id); onToggle(item.id); if (onClose && !item.subItems?.length) onClose(); }}
                pathname={pathname}
              />
            ))}
          </div>
...
function UserFooter() {
  ...
  return (
    <div className="flex-shrink-0 mx-3 mb-4 mt-2 p-3 rounded-xl border flex items-center gap-3" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>
      <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 border" style={{ backgroundColor: `${BRAND.accent}20`, borderColor: `${BRAND.accent}30` }}>
        <User size={14} style={{ color: BRAND.accent }} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-white text-xs font-semibold truncate">Staff Member</div>
        <div className="text-xs truncate" style={{ color: BRAND.muted }}>Accountant</div>
      </div>
      <button
        type="button"
...
function IconRail({ activeItem, onItemClick, isDetailExpanded, onToggleDetail }: {
  ...
  const allItems = STAFF_NAV.flatMap((s) => s.items);
...
export function StaffSidebar() {
  const pathname = usePathname();
  const [activeItem, setActiveItem] = useState(() => staffItemFromPath(pathname));
  const [isDetailExpanded, setIsDetailExpanded] = useState(true);
  const [isMobileOpen, setIsMobileOpen] = useState(false);
  const [openSubMenu, setOpenSubMenu] = useState<string | null>(null);

  useEffect(() => {
    setActiveItem(staffItemFromPath(pathname));
  }, [pathname]);
