...
import { useSignInMutation } from "@/lib/accubridge/api/authApi";

function deriveRole(apiRole: unknown): { role: string; country: string; dest: string } {
  const s = String(apiRole ?? "").toLowerCase();
  if (s === "admin" || s === "superadmin") return { role: "admin",  country: "both",    dest: "/accubridge/admin/dashboard"  };
  if (s === "staff" || s === "accountant") return { role: "staff",  country: "both",    dest: "/accubridge/staff/dashboard"  };
  return                                          { role: "client", country: "uk",      dest: "/accubridge/client/dashboard" };
}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="w-full text-center mt-2">
              <span className="text-xs text-[#6B7280]">
                ...
                <a href="/accubridge/(auth)/signup" className="underline text-[#D4AF37] hover:text-white transition">
                  Create one free
                </a>
              </span>
            </div>
