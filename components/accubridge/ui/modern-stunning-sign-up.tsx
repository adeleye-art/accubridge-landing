...
import { SignUpFormData } from "@/types/accubridge/auth";
import { useCheckEmailMutation, useSignUpMutation } from "@/lib/accubridge/api/authApi";
...
const SignUp = () => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      router.push("/accubridge/onboarding");
    } catch (err: unknown) {
      const msg =
        (err as { data?: { message?: string } })?.data?.message ??
